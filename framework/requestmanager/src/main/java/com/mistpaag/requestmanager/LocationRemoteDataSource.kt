...
class LocationRemoteDataSource @Inject constructor(
    ...
    override fun getLocationInfo(woeid: Long): Flow<ResultsData<WLocationInfo>> = flow {
        emit(
            renderLocationInfoData(
                safeApiCall(
                    call = {
                        locationService.getLocationInfo(woeid).callServices()
                    },
                    errorMessage = errorMessage
                )
            )
    ...
    private fun renderLocationInfoData(responseData: ResultsData<WLocationInfoServer>): ResultsData<WLocationInfo>{
        return when(responseData){
            is ResultsData.Error -> ResultsData.Error(responseData.message)
            is ResultsData.Success -> ResultsData.Success(responseData.data.toDomain())
        }
